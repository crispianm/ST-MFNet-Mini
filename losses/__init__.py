...
class DistillationLoss(nn.modules.loss._Loss):
    def __init__(self, args):
        ...
        for loss in args.distill_loss_fn.split("+"):
            ...
            if distill_loss_type == "MSE":
                ...
                module = import_module("losses.adversarial")
                distill_loss_function = getattr(module, "Adversarial")(
        ...
        for loss in args.student_loss_fn.split("+"):
            ...
            if student_loss_type == "MSE":
                ...
                module = import_module("losses.adversarial")
                student_loss_function = getattr(module, "Adversarial")(
                    args, student_loss_type
                )
    ...
    def forward(self, student_output, teacher_output, gt, input_frames):
        ...
        # distillation loss function: (alpha * student_loss) + distill_loss

        losses = []
        for l in self.loss:

            if l["student_function"] is not None:
                if l["student_type"] in ["T_WGAN_GP", "FI_GAN", "FI_Cond_GAN", "STGAN"]:
                    student_loss = l["student_function"](
                        student_output["frame1"], gt, input_frames
                    )
                else:
                    student_loss = l["student_function"](student_output["frame1"], gt)

            if l["distill_function"] is not None:
                if l["distill_type"] in ["T_WGAN_GP", "FI_GAN", "FI_Cond_GAN", "STGAN"]:
                    distill_loss = l["distill_function"](
                        student_output["frame1"], teacher_output["frame1"], input_frames
                    )
                else:
                    distill_loss = l["distill_function"](
            ...
            effective_loss = (self.alpha * student_loss) + distill_loss
            losses.append(effective_loss)
