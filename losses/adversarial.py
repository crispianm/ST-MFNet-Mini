...
class Adversarial(nn.Module):
    def __init__(self, args, gan_type):
        ...
        if gan_type == "T_WGAN_GP":
            ...
            self.discriminator = discriminator.FI_Cond_Discriminator(args)
        elif gan_type == "STGAN":
            self.discriminator = discriminator.ST_Discriminator(args)
        else:
        ...
        if gan_type != "WGAN_GP" and gan_type != "T_WGAN_GP":
            ...
            self.optimizer = optim.Adam(
                self.discriminator.parameters(), betas=(0, 0.9), eps=1e-8, lr=1e-5
            )
    ...
    def forward(self, fake, real, input_frames=None):
        ...
        for _ in range(self.gan_k):
            ...
            if self.gan_type in ["T_WGAN_GP", "FI_Cond_GAN", "STGAN"]:
                d_fake = self.discriminator(
                    input_frames[0], fake_detach, input_frames[1]
                )
            ...
            if self.gan_type in ["GAN", "FI_Cond_GAN", "STGAN"]:
                ...
                loss_d = F.binary_cross_entropy_with_logits(
                    d_01, label_01
                ) + F.binary_cross_entropy_with_logits(d_12, label_12)
        ...
        if self.gan_type == "GAN":
            ...
            loss_g = d_01_for_g * torch.log(
                d_01_for_g + 1e-12
            ) + d_12_for_g * torch.log(d_12_for_g + 1e-12)
            loss_g = loss_g.mean()
