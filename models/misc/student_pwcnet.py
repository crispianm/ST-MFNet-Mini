...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        class Extractor(torch.nn.Module):
            def __init__(self, channels=[12, 20, 32, 46, 50]):
        ...
        class Decoder(torch.nn.Module):
            def __init__(self, intLevel):
                ...
                self.netFiv = torch.nn.Sequential(
                    torch.nn.Conv2d(
                        in_channels=intCurrent + 128 + 96 + 64,
                        out_channels=32,
                        kernel_size=3,
                        stride=1,
                        padding=1,
    ...
    def forward(self, tenFirst, tenSecond, *args):
        ...
        objEstimate = self.netFiv(tenFirst[-1], tenSecond[-1], None)
        objEstimate = self.netFou(tenFirst[-2], tenSecond[-2], objEstimate)
        objEstimate = self.netThr(tenFirst[-3], tenSecond[-3], objEstimate)
        objEstimate = self.netTwo(tenFirst[-4], tenSecond[-4], objEstimate)
